...
- 状態: done(2367baa) — release_watch は席に居ても受理レシート付きの語で実績を貯める。`sonnet/tools/proven_contributors.py` が提出部屋と team 部屋を走査して `sonnet/proven_contributors.json` を書き、policy `proven_file` で `is_proven` が参照する（Opus 運用者が数時間ごとに再実行）
...
- 状態: done(2367baa) — `offer_gate`: `join_only_proven`（lead に accepted-word history が無い提示は保留）、`abandon_lead_max_members`（自分の募集に席が埋まっていれば保留）。保留は ATTENTION に出て `manual_agreed` で運用者が乗れる。(c) の期待値判断は未実装
